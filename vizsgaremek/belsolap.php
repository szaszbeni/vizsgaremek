        ...
        <tr>
              ...
              <ul id="baratok">
                <?php
                  ...
                  while ($friend = mysqli_fetch_assoc($my_friends)) {
                    echo "<li><a href='?friends={$friend['fname']}'>{$friend['fname']}</a></li>";
                  }
                ?>
              </ul>
            </div>
          ...
          <div id="leir" class="col-md-3">
            ...
            if (isset($_GET["friend"])) { 
              $friend_name = mysqli_real_escape_string($adb, $_GET["friend"]);
          
              $sql_info = "SELECT * FROM friends WHERE fname = '$friend_name'";
              $result_info = mysqli_query($adb, $sql_info);
              if (!$result_info) {
                  die("Hiba az adatlekérésben (friends): " . mysqli_error($adb));
              }
          
              if (mysqli_num_rows($result_info) > 0) {
                  $friend_info = mysqli_fetch_assoc($result_info);
                  
                  echo "<p><b>Barát neve:</b> {$friend_info['fname']}</p>";
                  echo "<p><b>Felhasználó ID:</b> {$friend_info['fid']}</p>";
