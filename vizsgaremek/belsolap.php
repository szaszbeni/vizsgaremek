            ...
            <div class="col-md-3">
              <ul id="baratok">
                <?php
                  ...
                  $my_friends = mysqli_query($adb, "SELECT fname FROM friends WHERE fStatus = 1");

                  while ($friend = mysqli_fetch_assoc($my_friends)) {
                      echo "<li>{$friend['fname']}</li>";
                  }
                  ?>
              </ul>
            </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["uzi"])) {
    ...
    if ($adb) {
        ...
        $sql = "INSERT INTO uzenetek (uDate, uEmail, uUzenet) 
    ...
    echo "<script>
        ...
        newMessage.innerHTML = '<b>Dátum:</b> ' + '$date' + '<br><b>Felhasználó:</b> ' + '$email' + '<hr><b>Üzenet:</b> ' + '$message' + '<br><br>';
